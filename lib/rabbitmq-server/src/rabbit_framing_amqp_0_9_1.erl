...
-type(amqp_method_field_name() ::
       ( durable | exchange | requeue | routing_key
       | ticket | consumer_tag | destination | reply_code
       | source | routing_key | delivery_tag | nowait
       | arguments | delivery_tag | redelivered | exchange
       | ticket | internal | routing_key | message_count
       | ticket | queue | passive | durable
       | queue | exclusive | ticket | auto_delete
       | multiple | nowait | arguments | delivery_tag
       | exchange | arguments | queue | message_count
       | consumer_count | requeue | ticket | no_local
       | routing_key | queue | immediate | exchange
       | routing_key | nowait | arguments | delivery_tag
       | queue | mandatory | no_ack | requeue
       | ticket | prefetch_count | queue | nowait
       | exclusive | no_ack | frame_max | message_count
       | consumer_tag | ticket | exchange | queue
       | if_unused | if_empty | exchange | nowait
       | arguments | message_count | reply_code | nowait
       | reply_text | channel_max | class_id | read
       | method_id | arguments | out_of_band | consumer_tag
       | reply_text | channel_id | active | prefetch_size
       | active | nowait | exchange | global
       | ticket | reply_code | reply_text | class_id
       | redelivered | method_id | ticket | queue
       | requeue | routing_key | consumer_tag | multiple
       | realm | exclusive | passive | ticket
       | active | destination | write | version_major
       | version_minor | server_properties | mechanisms | locales
       | consumer_tag | ticket | capabilities | client_properties
       | mechanism | response | locale | auto_delete
       | insist | challenge | exchange | nowait
       | response | routing_key | channel_max | ticket
       | type | cluster_id | heartbeat | delivery_tag
       | if_unused | nowait | nowait | frame_max
       | heartbeat | ticket | known_hosts | virtual_host
