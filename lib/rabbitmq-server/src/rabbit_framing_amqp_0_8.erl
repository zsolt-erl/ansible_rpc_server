...
-type(amqp_method_field_name() ::
       ( exchange | table | consumer_tag | reply_code
       | reply_text | dtx_identifier | details | identifier
       | reply_code | delivery_tag | reply_text | ticket
       | consumer_tag | class_id | consumer_tag | method_id
       | queue | consume_rate | exchange | routing_key
       | delivery_tag | version_major | version_minor | nowait
       | integer_result | ticket | exclusive | content_size
       | mechanisms | active | meta_data | write
       | integer_3 | client_properties | mechanism | challenge
       | immediate | response | queue | routing_key
       | locale | string_result | exchange | consumer_tag
       | consumer_tag | ticket | exchange | response
       | routing_key | prefetch_count | routing_key | passive
       | cluster_id | mandatory | channel_max | auto_delete
       | nowait | internal | consumer_tag | nowait
       | channel_max | type | delivery_tag | consumer_tag
       | frame_max | reply_code | heartbeat | routing_key
       | string_op | ticket | virtual_host | integer_2
       | capabilities | global | insist | reply_text
       | routing_key | known_hosts | integer_op | host
       | known_hosts | consumer_tag | passive | durable
       | reply_code | durable | class_id | exclusive
       | delivery_tag | auto_delete | method_id | arguments
       | requeue | ticket | nowait | queue
       | message_count | consumer_count | exchange | no_local
       | exchange | reply_code | active | ticket
       | heartbeat | exchange | queue | delivery_tag
       | global | exchange | integer_1 | multiple
       | routing_key | nowait | arguments | routing_key
       | consumer_tag | queue | nowait | prefetch_size
       | requeue | ticket | exchange | queue
       | arguments | no_ack | nowait | ticket
       | ticket | mandatory | no_local | exchange
       | message_count | mandatory | immediate | immediate
       | realm | out_of_band | ticket | string_2
       | string_1 | queue | routing_key | reply_code
       | if_unused | redelivered | if_empty | nowait
       | ticket | prefetch_count | redelivered | operation
       | content_checksum | server_properties | requeue | message_count
       | identifier | consumer_tag | delivery_tag | ticket
       | queue | passive | exchange | delivery_tag
       | routing_key | reply_text | frame_max | arguments
       | redelivered | no_ack | exclusive | operation
       | nowait | queue | exchange | locales
       | exclusive | reply_text | consumer_tag | identifier
       | consumer_tag | integer_4 | delivery_tag | prefetch_size
       | ticket | prefetch_count | reply_text | requeue
       | global | if_unused | multiple | queue
       | consumer_tag | staged_size | result | ticket
       | nowait | read | active | consumer_tag
       | no_local | no_ack | exchange | exclusive
       | queue | nowait | routing_key | message_count
       | prefetch_size | nowait | consumer_tag | ticket
       | exchange | result )).
